...
istream& operator >> (istream &is, TBigUInt &bui) {
    ...
    for (int32_t i = str.length(); i > 0; i -= TBigUInt::NUM_SIZE) {
        bui.number.push_back(stoull(i < (int32_t)TBigUInt::NUM_SIZE ? str.substr(0, i) : str.substr(i - TBigUInt::NUM_SIZE, TBigUInt::NUM_SIZE)));
    }
    bui.FixZero();
    return is;
}
...
TBigUInt TBigUInt::operator / (const TBigUInt& bui) {
    ...
    for (int32_t i = u.number.size() - 1; i >= 0; --i) {
        cur = cur * base + u.number[i];
        int64_t num1 = cur.number.size() <= v.number.size() ? 0 : cur.number[v.number.size()];
        int64_t num2 = cur.number.size() <= v.number.size() - 1 ? 0 : cur.number[v.number.size() - 1];
        int64_t num = (num1 * BASE + num2) / v.number.back();
